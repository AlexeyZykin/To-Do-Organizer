...
class ListOfEventsFragment : Fragment(), EventAdapter.EventListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fabCreateEvent.setOnClickListener {
            val action = ListFragmentDirections.actionListFragmentToAddEditEventFragment(null, listItem)
...
//        Log.d("TAG", "resume")
    ...
    override fun onCLickItem(event: Event, state: Int) {
        when(state){
            EventAdapter.EDIT -> {
                val action = ListFragmentDirections.actionListFragmentToAddEditEventFragment(event, null)
                findNavController().navigate(action)
            }
            EventAdapter.DELETE -> {
                deleteEvent(event)
