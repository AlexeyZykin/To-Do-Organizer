...
class ListOfTasksFragment : Fragment(), TaskAdapter.TaskListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fabNewTask.setOnClickListener {
            val action = ListFragmentDirections.actionListFragmentToAddEditTaskFragment(null, listItem)
...
//        Log.d("TAG", "resume")
    ...
    override fun onCLickItem(task: Task, state: Int) {
        when(state){
            ...
            TaskAdapter.EDIT -> {
                val action = ListFragmentDirections.actionListFragmentToAddEditTaskFragment(task, null)
                findNavController().navigate(action)
            }
            TaskAdapter.STAR ->{
                viewModel.updateTask(task)
