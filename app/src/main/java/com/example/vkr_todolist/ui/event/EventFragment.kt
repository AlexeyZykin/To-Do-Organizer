...
import com.example.vkr_todolist.ui.task.TaskFragmentDirections
...
class EventFragment : Fragment(), EventAdapter.EventListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fabCreateEvent.setOnClickListener {
            val action = EventFragmentDirections.actionEventFragmentToAddEditEventFragment(event = null, listName = null)
        ...
        (requireActivity() as MenuHost).addMenuProvider(object: MenuProvider {
            ...
            override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
                when(menuItem.itemId){
                    R.id.searchFragment-> {
                        val action = EventFragmentDirections.actionEventFragmentToSearchFragment(EVENT)
                        findNavController().navigate(action)
                        return true }
    ...
    override fun onCLickItem(event: Event, state: Int) {
        when(state){
            EventAdapter.EDIT -> {
                val action = EventFragmentDirections.actionEventFragmentToAddEditEventFragment(event, null)
                findNavController().navigate(action)
            }
            EventAdapter.DELETE -> {
                deleteEvent(event)
