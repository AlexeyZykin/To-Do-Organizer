package com.example.vkr_todolist.ui.search
...
import com.example.vkr_todolist.adapter.EventAdapter
...
import com.example.vkr_todolist.ui.event.EventFragment
import com.example.vkr_todolist.ui.event.EventFragmentDirections
import com.example.vkr_todolist.ui.main.MainViewModel
import com.example.vkr_todolist.ui.note.NoteFragment
...
class SearchFragment : Fragment(), TaskAdapter.TaskListener, NoteAdapter.NoteListener, EventAdapter.EventListener {
    ...
    override fun onCLickItem(note: Note, state: Int) {
        when(state){
            NoteAdapter.EDIT -> {
                val action = SearchFragmentDirections.actionSearchFragmentToAddEditNoteFragment(note, null)
                findNavController().navigate(action)
            }
            NoteAdapter.DELETE -> {
                deleteNote(note)
    ...
    override fun onCLickItem(event: Event, state: Int) {
        when(state){
            EventAdapter.EDIT -> {
                val action = SearchFragmentDirections.actionSearchFragmentToAddEditEventFragment(event, null)
                findNavController().navigate(action)
            }
            EventAdapter.DELETE -> {
                deleteEvent(event)
